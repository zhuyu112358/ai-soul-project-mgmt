...
**最后更新：** 2026-09-05（第十二轮监控，BUG-003关闭，BUG-006崩溃已修复/退化待优化，BUG-007部分修复，BUG-008已修复）
**维护者：** 总体监控任务
...
- **发现者：** 集成测试任务
- **负责方：** SoulArena
- **状态：** 🟡 修复中（崩溃已修复，运行时退化待优化，2026-09-05 第十二轮监控）
...
- **发现者：** 集成测试任务
- **负责方：** Seed
- **状态：** 🟡 修复中（2026-09-05 第十二轮监控，6个失败→2个失败，部分修复）
- **问题描述：** AcousticPropagation衍射功能测试失败，衍射强度始终为0，角度衰减逻辑反转，系数无效果。
- **修复进展：** Seed d395293实现了声学衍射功能（sound bending around occluder corners），测试从444→482（+38），6个衍射失败降至2个失败。剩余2个失败需进一步调试。
- **当前测试状态：** 482测试，480通过，2失败（第十二轮监控实测）
- **修复commit：** d395293（acoustic diffraction实现）
- **回归验证：** 待2个剩余失败修复后验证
...
- **发现者：** 集成测试任务
- **负责方：** Seed
- **状态：** ✅ 已关闭（2026-09-05 第十二轮监控，Seed 46b3191已修复）
- **修复内容：** Seed 46b3191新增灵魂过滤（soul filtering），discoverSouls增加status==='active'过滤。同时完成3灵魂集成测试。
- **修复commit：** 46b3191（SDK v1.0.0 tag + 3-soul integration test + soul filtering）
- **回归验证：** 3灵魂集成测试通过（Seed 46b3191）
...
### BUG-008: Seed集成测试选择sleeping灵魂 — ✅ 已关闭（46b3191修复）
- discoverSouls增加status==='active'过滤，3灵魂集成测试通过
