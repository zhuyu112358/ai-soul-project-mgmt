...
- **发现者：** 集成测试任务
- **负责方：** SoulArena
- **状态：** 🔴 已派发（2026-09-05 第十一轮监控，派发SoulArena开发任务，**最高优先级**）
- **问题描述：** 服务器运行约15-20分钟后进程完全退出。导致BUG-001（并发400）和BUG-002（Nova超时）的退化症状。干净服务器上所有功能正常，证明是运行时退化/崩溃问题。
- **监控数据（第4轮，21:05）：** 服务器稳定运行770秒（~13分钟）未崩溃。内存100.9MB→104MB（增长~3MB，增长率1.15MB/分钟）。perceive延迟Vex 60ms/Nova 60ms（干净服务器27ms/26ms，增长2.2倍）。预计18分钟时达~110MB（接近崩溃阈值）。
- **修复要求（按优先级）：**
  1. **立即：** 添加进程守护（pm2），崩溃后自动重启
  2. **短期：** 添加未捕获异常处理（uncaughtException/unhandledRejection），记录错误日志
  3. **中期：** 排查内存泄漏（认知子系统状态累积、事件监听器未清理、LLMScheduler资源未释放）
  4. **长期：** 添加健康检查端点和运行时监控
- **修复commit：** （待修复）
- **回归验证：** （待验证：服务器持续运行30分钟以上不崩溃，内存稳定）
