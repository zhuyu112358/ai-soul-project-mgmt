...
- 灵魂系统：SoulArena v4.90，100认知子系统（里程碑），BehaviorArbiter行为仲裁，SDK v1.0.0已发布，197个持久化单元测试，LLMScheduler超时/队列过期/并发提升，Guardian v3进程守护（可靠端口绑定+TIME_WAIT等待+双熔断器自动恢复+残留进程清理），内存优化修复49无界数组，uncaughtException null-err保护+crash日志前置
...
- 已知问题：BUG-006根因已修复v4.90（uncaughtException null-err导致静默退出），门控仅剩30分钟稳定性验证；但Nova退化加速到2分钟仍需优化（M2目标<500ms）；bug 7关闭/2活跃；管理策略更新（里程碑+合规+文档机制）
